...
import { init, BtcutilSync } from 'btcutil-js';
import {
  buildWatchOnlyPsbt,
  bytesToHex,
...
describe('watch-only PSBT construction', () => {
  ...
  beforeAll(async () => {
    const wasm = await readFile('node_modules/btcutil-js/dist/btcutil.wasm');
    btc = await init(wasm.buffer.slice(wasm.byteOffset, wasm.byteOffset + wasm.byteLength) as ArrayBuffer);
  });
  ...
  it('uses a full non-witness UTXO for a legacy descriptor', () => {
    ...
    expect(decoded.inputs[0].nonWitnessUtxo && bytesToHex(decoded.inputs[0].nonWitnessUtxo)).toBe(bytesToHex(previousTx));

...
export interface ScanProgress {
  phase: 'deriving' | 'fetching' | 'finalizing' | 'done';
  addressesDerived: number;
  txsFound: number;
  chain?: Chain;
  batchAddresses?: number;
  apiRequests?: number;
 ...
 *
 * Batch behavior verified against the Mempool API:
...
export class WalletScannerService {
   ...
   */
  async scan(request: WalletScanRequest): Promise<{ wallet: WatchWallet; txs: Transaction[]; truncated: boolean }> {
    const { descriptor, scriptType, network, gapLimit, source, label, identity, onProgress, minimumDerived, existingWalletId } = request;
    ...
    const extend = async (chain: Chain): Promise<DerivedAddress[]> => {
      onProgress?.({
        phase: 'deriving',
        addressesDerived: addresses.length,
        txsFound: txsById.size,
        chain,
        apiRequests: pagesUsed,
        addressesQueried,
      });
    ...
    const fetchFor = async (targets: DerivedAddress[]): Promise<void> => {
      ...
      const chain = targets[0].chain;
      addressesQueried += targets.length;
      ...
      for (;;) {
        ...
        onProgress?.({
          ...
          txsFound: txsById.size,
          chain,
          batchAddresses: targets.length,
          apiRequests: pagesUsed + 1,
          addressesQueried,
        });

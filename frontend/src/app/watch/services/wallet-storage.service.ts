...
import { Bip329Label, Bip329LabelType, WatchNetwork, WatchWallet } from '../watch.types';
...
interface WalletLocalState {
  ...
  /** Legacy name retained only so existing browser state migrates cleanly. */
  descriptorNoticeDismissed?: boolean;
}
 ...
 * That deliberate tradeoff allows the gap limit to auto-extend without re-prompting the user.
 *
...
export class WalletLocalStateService {
  ...
  getShowAddressDerivationPath(wallet: WatchWallet): boolean {
    return this.load(wallet).showAddressDerivationPath === true;
  }

  setShowAddressDerivationPath(wallet: WatchWallet, show: boolean): void {
    const state = this.load(wallet);
    state.showAddressDerivationPath = show;
    this.persist(wallet, state);
  }

  getDescriptorOriginAcknowledged(wallet: WatchWallet): boolean {
    const state = this.load(wallet);
    return state.descriptorOriginAcknowledged === true || state.descriptorNoticeDismissed === true;
  }

  setDescriptorOriginAcknowledged(wallet: WatchWallet, acknowledged: boolean): void {
    const state = this.load(wallet);
    state.descriptorOriginAcknowledged = acknowledged;
    delete state.descriptorNoticeDismissed;
    this.persist(wallet, state);
  }
  ...
  private read(key: string): WalletLocalState {
    ...
    try {
      ...
      return {
        ...
        showAddressDerivationPath: parsed.showAddressDerivationPath === true,
        descriptorOriginAcknowledged: parsed.descriptorOriginAcknowledged === true
          || parsed.descriptorNoticeDismissed === true,
        descriptorNoticeDismissed: parsed.descriptorNoticeDismissed === true,
      };
    } catch {
      return { frozen: [] };
    }

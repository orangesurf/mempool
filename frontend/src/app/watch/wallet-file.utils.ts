...
function descriptorFromText(value: string): string | null {
  ...
  for (const rawLine of value.replace(/\r/g, '').split('\n')) {
    ...
    // `match[0]` includes the opening parenthesis and may also include one boundary
    // character. Locate the captured descriptor function within that full match instead
    // of deriving its offset from string lengths: the old calculation dropped the first
    // character of a descriptor at column zero (`wpkh(` became `pkh(`), leaving Sparrow's
    // otherwise-valid checksum attached to a different descriptor.
